        ...
        <div class="mt-2">
          ...
          <div class="flex items-center gap-1 mt-1">
            <span class="material-symbols-outlined text-primary text-[18px]"
              >people</span
            >
            <p class="text-primary text-xs font-semibold">Eligible semhas</p>
          </div>
        </div>
      </div>
        ...
        <div class="mt-2">
          ...
          <div class="flex items-center gap-1 mt-1">
            <span class="material-symbols-outlined text-green-600 text-[18px]"
              >check_circle</span
            >
            <p class="text-green-600 text-xs font-semibold">Terjadwal semhas</p>
          </div>
        </div>
      </div>
        ...
        <table class="w-full text-left text-sm whitespace-nowrap">
          ...
          >
            <tr>
              ...
              <th class="px-6 py-4">Jadwal Semhas</th>
          ...
          <tbody class="divide-y divide-border-light">
            ...
            >
              ...
              <td class="px-6 py-4">
                <template v-if="item.is_scheduled && item.semhas_seminar">
                  <div class="flex items-center gap-2 text-sm text-text-main">
                    <span
                      class="material-symbols-outlined text-[18px] text-green-600"
                      >event_available</span
                    >
                    {{ formatDate(item.semhas_seminar.tanggal) }}
                  </div>
                  <p class="text-xs text-text-secondary mt-0.5">
                    {{ item.semhas_seminar.waktu || "" }}
                    {{
                      item.semhas_seminar.ruangan
                        ? "• " + item.semhas_seminar.ruangan
                        : ""
                    }}
                  </p>
              ...
              <td class="px-6 py-4 text-right">
                ...
                >
                  ...
                  <button
                    v-if="item.is_scheduled"
                    @click="viewDetail(item.semhas_seminar)"
...
const fetchSeminarHasil = async () => {
  try {
    ...
    const response = await adminService.getSeminarHasil(params);
    if (response.success) {
    ...
    console.error("Failed to fetch seminar hasil data:", error);
  } finally {
    loading.value = false;
  }
};
...
const fetchStats = async () => {
  try {
    const [terjadwalRes, belumRes] = await Promise.all([
      adminService.getSeminarHasil({ jadwal: "terjadwal", per_page: 1 }),
      adminService.getSeminarHasil({ jadwal: "belum", per_page: 1 }),
    ]);
...
const saveSeminarSchedule = async () => {
  try {
    saving.value = true;
    await adminService.createSeminarHasil({
...
const viewDetail = (seminar) => {
  if (seminar && seminar.id) {
    router.push(`/admin/seminarhasil/${seminar.id}`);
  }
};
...
const getStatusClass = (status) => {
  const classes = {
    bimbingan: "bg-yellow-50 text-yellow-600 border border-yellow-100",
    semhas: "bg-blue-50 text-blue-600 border border-blue-100",
    ujian: "bg-purple-50 text-purple-600 border border-purple-100",
  };
